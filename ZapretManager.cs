...
namespace SerpiumVPN
{
    public class ZapretManager
    {
        ...
        /// </summary>
        public async Task<bool> AutoSelectStrategyAsync(bool checkYoutube, bool checkDiscord)
        {
            ...
            {
                MessageBox.Show(
                    $"Не найдено ни одного general*.bat.\n\nПроверь папку:\n{_batsPath}",
                    "Автоподбор",
                    MessageBoxButton.OK,
            ...
            foreach (string batPath in batFiles)
            {
                ...
                try
                {
                    ...
                    bool isNetworkWorking = await CheckConnectionAsync(checkYoutube, checkDiscord);

                    if (isNetworkWorking)
                    {
                        Debug.WriteLine("--------------------------------------------------");
                        Debug.WriteLine($"[AUTO SUCCESS] Рабочая стратегия: {batFileName}");
                        Debug.WriteLine("--------------------------------------------------");

                        CurrentStrategyName = batFileName;

                        MessageBox.Show(
                            $"Успешно подобран рабочий конфиг:\n{batFileName}",
                            "Автоподбор",
                            MessageBoxButton.OK,
                    ...
                    Debug.WriteLine($"[AUTO FAIL] Проверка сервисов не прошла: {batFileName}");
                    Stop();
                }
                catch (Exception ex)
                {
        ...
        public async Task<bool> CheckConnectionAsync(bool checkYoutube, bool checkDiscord, int timeoutMs = 3000)
        {
            if (!checkYoutube && !checkDiscord)
            {
                ...
                return await QuickTestUrlAsync("https://1.1.1.1", timeoutMs);
            }
            ...
            if (checkYoutube)
            {
                ...
                if (!await QuickTestUrlAsync("https://www.youtube.com", timeoutMs))
                {
                    Debug.WriteLine("[CHECK FAIL] YouTube недоступен.");
                    return false;
                }

                Debug.WriteLine("[CHECK OK] YouTube доступен.");
            }
            ...
            if (checkDiscord)
            {
                ...
                if (!await QuickTestUrlAsync("https://discord.com", timeoutMs))
                {
                    Debug.WriteLine("[CHECK FAIL] Discord недоступен.");
                    return false;
                }

                Debug.WriteLine("[CHECK OK] Discord доступен.");
            }
        ...
        private void StartStrategyFromBatPath(string batPath)
        {
            ...
            _ = Task.Run(async () =>
            {
                ...
                try
                {
                    ...
                    {
                        Debug.WriteLine($"[LAUNCH WARN] winws.exe сразу завершился. ExitCode: {_zapretProcess.ExitCode}");
                        File.AppendAllText(
                            logPath,
                            $"{Environment.NewLine}winws.exe exited quickly. ExitCode: {_zapretProcess.ExitCode}{Environment.NewLine}",
                            Encoding.UTF8
                        );
                    }
                }
